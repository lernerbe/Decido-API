...
router = APIRouter(prefix="/users", tags=["users_and_teams"])

################################## USERS ####################################

# Get all users with paginationtestin
@router.post("/")
...
def update_user(user:Annotated[dict,Depends(get_current_user)],user_id: int,user_update_body: UserBody) -> UserResponse: 

    if user is None or user.get("user_role") not in ["manager","admin"]:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED,detail="Could not validate credentials make sure you have admin permisson")
    try:     
        return User.udpate_user_by_id(user_id,user_update_body)
...
def delete_user(user:Annotated[dict,Depends(get_current_user)],user_id: int) -> str: 
...
def get_team(user:Annotated[dict,Depends(get_current_user)],team_id: int) -> TeamResponse: 
    print(user)
    if user is None or user.get("user_role") not in ["manager","admin"]:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED,detail="Could not validate credentials make sure you have admin permisson")
    try:
...
# Update team by id
@router.put("/edit_team/{team_id}")
def update_team(user:Annotated[dict,Depends(get_current_user)],team_id: int,team: TeamBody) -> TeamResponse: 
    print(user)
    if user is None or user.get("user_role") not in ["manager","admin"]:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED,detail="Could not validate credentials make sure you have admin permisson")
    try:
        return Team.udpate_team_by_id(team_id,team)
...
def delete_team(user:Annotated[dict,Depends(get_current_user)],team_id: int) -> Response: 
    print(user)
    if user is None or user.get("user_role") not in ["manager","admin"]:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED,detail="Could not validate credentials make sure you have admin permisson")
    try:
